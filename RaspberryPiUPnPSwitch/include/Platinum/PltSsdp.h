...
class PLT_SsdpAddrConfig
{
private:
    NPT_String m_SsdpGroupIpAddr;
    NPT_UInt16 m_SsdpPort;

public:
    static PLT_SsdpAddrConfig* Instance();

public:
    NPT_String  GetGroupIpAddr();
    NPT_UInt16  GetPort();
    NPT_String  GetGroupIpAddrAndPort();
    NPT_HttpUrl GetSsdpGroupUrl();

    void SetGroupIpAddr(const NPT_String& addr);
    void SetPort(NPT_UInt16 port);
...
public:
    ...
    NPT_Result operator()(NPT_IpAddress& if_addr) const {
        NPT_IpAddress addr;
        addr.ResolveName(PLT_SsdpAddrConfig::Instance()->GetGroupIpAddr());
